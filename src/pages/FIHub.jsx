...
const GlassCard = ({ children, className = "" }) => (
  <div className={cn("rounded-2xl border border-white/[0.07] p-4", className)}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        {canWrite && (
          <Button onClick={() => setShowAjouter(true)} className="bg-blue-600/80 hover:bg-blue-600 border border-blue-500/30 text-white gap-2 h-9">
        ...
        {fi && (
          <div className="flex flex-col items-end gap-2">
            ...
            <div className="text-right">
              <p className="text-xs text-zinc-500">{fi.campus}{fi.pilote_nom ? ` · ${fi.pilote_nom}` : ""}</p>
            </div>
          </div>
        )}
      </div>

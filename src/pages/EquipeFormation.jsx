...
import { GraduationCap, Mail, Shield, BookOpen } from "lucide-react";
...
const ROLE_LABELS = {
  responsable_formation: "Responsable du Pôle",
...
function MemberCard({ user, onClick }) {
  ...
  const gradient = group?.color || "from-violet-500 to-purple-600";
  ...
  return (
    <button onClick={onClick} className="ai-card p-5 flex flex-col items-center gap-3 text-center card-hover w-full">
      ...
      <div>
        ...
        <p className="text-xs text-violet-400 font-medium mt-0.5">{ROLE_LABELS[user.role] || user.role}</p>
...
export default function EquipeFormationPage() {
  useTrackActivity("EquipeFormation");
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-10">
      <div className="mb-8 flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-white">Équipe — Formation</h1>
          <p className="text-sm text-zinc-500 mt-0.5">Pôle 2 — L'École des Leaders</p>
        </div>
      </div>

      <MemoFlashBoard pole="formation" user={currentUser} />
